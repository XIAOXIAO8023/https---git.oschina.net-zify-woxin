...
@implementation LNAnimationNumber
{
    CGFloat _proNumber;
}
...
- (void)showInitContent
{
    if (_numberOfDecimalPlaces == numberOfDecimalPlacesZero) {
        self.text = [NSString stringWithFormat:@"%@ %.0f%@ %@",_foreString,0.0,_specialCharacters,_followString];
    }else if (_numberOfDecimalPlaces == numberOfDecimalPlacesOne){
        self.text = [NSString stringWithFormat:@"%@ %.1f%@ %@",_foreString,0.0,_specialCharacters,_followString];
    }else{
        self.text = [NSString stringWithFormat:@"%@ %.2f%@ %@",_foreString,0.0,_specialCharacters,_followString];
    }
    
    if (isMultColor) {
        NSMutableArray *a = [self getRangesOfNumberInString:self.text];
        for (int i = 0; i < a.count; i++) {
            NSValue *v = [a objectAtIndex:i];
            NSRange r;
            [v getValue:&r];
            [self setFont:_numberFont textColor:_numberColor range:r];
        }
    }
}
...
-(void)animationWithThread
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [timer invalidate];
        if (_numberOfDecimalPlaces == numberOfDecimalPlacesZero) {
            self.text = [NSString stringWithFormat:@"%@ %.0f%@ %@",_foreString,_number,_specialCharacters,_followString];
        }else if (_numberOfDecimalPlaces == numberOfDecimalPlacesOne){
            self.text = [NSString stringWithFormat:@"%@%.1f%@ %@",_foreString,_number,_specialCharacters,_followString];
        }else{
            self.text = [NSString stringWithFormat:@"%@%.2f%@ %@",_foreString,_number,_specialCharacters,_followString];
        }
...
{
    _proNumber += [[timer userInfo] floatValue];
    if(_proNumber >= _number){
        _proNumber = _number;
    }
    
    if (_numberOfDecimalPlaces == numberOfDecimalPlacesZero) {
        self.text = [NSString stringWithFormat:@"%@ %.0f%@ %@",_foreString,_proNumber,_specialCharacters,_followString];
    }else if (_numberOfDecimalPlaces == numberOfDecimalPlacesOne){
        self.text = [NSString stringWithFormat:@"%@ %.1f%@ %@",_foreString,_proNumber,_specialCharacters,_followString];
    }else{
        self.text = [NSString stringWithFormat:@"%@ %.2f%@ %@",_foreString,_proNumber,_specialCharacters,_followString];
    }
